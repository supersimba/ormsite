...
logger=logging.getLogger('target_logging')
handler=RotatingFileHandler('../log/target-collectschd.log',maxBytes=30000,backupCount=2)
...
def CollectTgtMoniInfo():
    ...
    if len(args)>0:
        for r in args:
            ...
            sshpwd = r[3]
            scriptpath=r[4]
            logger.info('begin to check connection of ssh :%s' % tgtip)
            ...
            if check_list[0] == 1 and check_list[1] == 1 and check_list[2] == 1:
                ...
                try:
                    ...
                    if out:
                        try:
                            ...
                            c = conn.cursor()
                            c.execute(
                                    "insert into tgt_moni_info(tgt_ssh_status,tgt_path_status,exec_script_status,sync_status,active,collect_cnt,collect_err,loader_s_cnt,loader_r_cnt,loader_s_p_cnt,loader_r_p_cnt,loader_rate,loader_time,loader_err,queue_id,add_time) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                ...
                try:
                    ...
                    c = conn.cursor()
                    c.execute("insert into tgt_moni_info(tgt_ssh_status,tgt_path_status,exec_script_status,queue_id,add_time) values(%s,%s,%s,%s,%s)",
